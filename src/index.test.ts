...
describe('Core handleInput function', () => {
	...
	it('provides feedback to users and recovers when invalid characters and whitespace are used', () => {
		...
		expect(warnSpy).toHaveBeenCalledWith(
			'Warning: "f" is not a valid operator or operand. It will be ignored.'
		)
		expect(warnSpy).toHaveBeenCalledWith(
			'Warning: "e" is not a valid operator or operand. It will be ignored.'
		)
		expect(warnSpy).toHaveBeenCalledWith(
			'Warning: "d" is not a valid operator or operand. It will be ignored.'
		)
	})
	...
	it('provides feedback to users and recovers when an operation is attempted with insufficient operands', () => {
		...
		expect(errorSpy).toHaveBeenCalledWith(
			`Error: Operators require 2 operands to evaluate. The problematic operator "+" will be ignored \nPlease provide an additional operand before adding an operator.`
		)
	})
	...
	it('provides feedback to users and recovers when an operand has multiple decimal points', () => {
		...
		expect(warnSpy).toHaveBeenCalledWith(
			`Warning: Operands cannot contain more than one decimal. Only digits before the second decimal point in "5.0.0" will be evaluated`
		)
	})
})
